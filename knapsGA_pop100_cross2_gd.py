...
# グリーディ生成関数
def generate_greedy_individual(value, weight, capacity):
    n = len(value)
...
def generate_greedy_population(pop_size, value, weight, capacity):
    return [generate_greedy_individual(value, weight, capacity) for _ in range(pop_size)]
...
def genetic_algorithm():
    population = generate_greedy_population(popSize, value, weight, capacity) #①初期個体群の生成
    best_individual = max(population, key=fitness) #この後のエリート戦略で使用するために適応度が最大のものを抜き出す
    best_value = fitness(best_individual)
    best_weight = np.dot(best_individual, weight)
    best_values = [(best_value, best_weight)] # 最適な適応度と重さを保存するリスト
    ...
    for generation in range(generations):
        ...
        current_best_weight = np.dot(current_best_individual, weight)
        ...
        best_values.append((best_value, best_weight)) #各世代の最適な適応度と重さを保存
        
        print(f"Generation {generation + 1}:  Best Value = {best_value}")
        
    return best_individual, best_value, best_weight, best_values

#遺伝的アルゴリズムを実行し、最適解を取得
best_solution, best_value, best_weight, best_values = genetic_algorithm()
print("\nFinal Best Solution:")
pprint(best_solution)
print("Final Best Value:", best_value)
print("Final Best Weight:", best_weight)

#適応度の推移をプロット
values, weights = zip(*best_values)
plt.plot(values, label='Best Value')
plt.xlabel('Generation')
plt.ylabel('Best Value')
plt.title('Best Value and Best Weight over Generations')
plt.legend()
plt.show()
